...
class ToolDiscoveryManager:
    ...
    def _find_tool_classes(self, module: Any) -> List[Tool]:
        ...
        for name in dir(module):
            ...
            if (
                inspect.issubclass(obj) and 
                issubclass(obj, Tool) and 
                obj is not Tool and
                obj.__module__ == module.__name__ # makes sure that the module was created and not imported
            ):
                tools.append(obj)

        return tools

    def discover_from_file(self, directory: Path) -> None:
    ...
    def discover_all(self) -> None:
        self.discover_from_file(self.config.cwd)
        self.discover_from_file(get_config_dir())
